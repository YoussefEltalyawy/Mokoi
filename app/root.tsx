...
export async function loader(args: LoaderFunctionArgs) {
  ...
  const homeMeta = await args.context.storefront
    .query(HOME_METAOBJECT_QUERY, {
      variables: { handle: 'in-mokoi-we-trust' },
    })
    .catch(() => null);

  const announcmentText: string | null = (() => {
    const node = (homeMeta as any)?.metaobject ?? null;
    const fields = node?.fields ?? [];
    const f = fields.find((x: any) => x?.key === 'announcment');
  ...
  return (
    <html lang="en" className="h-full">
      <head>
        ...
        <link rel="stylesheet" href={marqueeStyles}></link>
